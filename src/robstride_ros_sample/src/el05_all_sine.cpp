...
int main(int argc,char **argv){
  bool ok=false, large_motion_confirmed=false, hold_enabled=false; double duration=60, amp=.02, amp_id2=-1.0, freq=.1, speed=.1;
  ...
  for(int i=1;i<argc;i++){std::string a=argv[i];
    ...
    else {std::cerr<<"Usage: el05_all_sine --confirm-hardware [--confirm-large-motion] [--hold-enabled] [--skip-id N] [--duration s] [--amplitude rad] [--amplitude-id2 rad] [--frequency Hz] [--speed rad/s]\n"; return 2;}}
  ...
  if(!ok||duration<=0||amp<=0||amp>max_amp||amp_id2<=0||amp_id2>max_amp||freq<=0||freq>.2||speed<=0||speed>.2){
  ...
  try { for(int i=0;i<5;i++){if(!active[i]) continue; m[i]=std::make_unique<RobStrideMotor>("can0",0xFD,(uint8_t)(i+1),0); m[i]->Get_RobStrite_Motor_parameter(0x7019); c[i]=m[i]->drw.mechPos.data; if(!std::isfinite(c[i])) throw std::runtime_error("invalid position feedback");}
    std::cout<<"Selected motors ready; skipped IDs:"; for(int i=0;i<5;i++) if(!active[i]) std::cout<<" "<<(i+1); std::cout<<"; amplitude="<<amp<<" rad; id2 amplitude="<<amp_id2<<" rad; duration="<<duration<<" s\n";
    auto start=std::chrono::steady_clock::now();
    while(!stop){double t=std::chrono::duration<double>(std::chrono::steady_clock::now()-start).count(); if(t>=duration)break; for(int i=0;i<5;i++) if(active[i]) { const double a = (i == 1) ? amp_id2 : amp; const double q=a*std::sin(2*M_PI*freq*t); m[i]->RobStrite_Motor_PosCSP_control((float)speed,(float)(c[i]+q)); } std::this_thread::sleep_for(std::chrono::milliseconds(20));}
    if (hold_enabled && !stop) {
      ...
      while (!stop) {
        for (int i=0;i<5;i++) if(active[i])
          m[i]->RobStrite_Motor_PosCSP_control((float)speed, (float)c[i]);
