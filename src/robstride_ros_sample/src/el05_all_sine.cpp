...
int main(int argc,char **argv){
  bool ok=false, large_motion_confirmed=false, hold_enabled=false; double duration=60, amp=.02, freq=.1, speed=.1;
  ...
  for(int i=1;i<argc;i++){std::string a=argv[i];
    ...
    else {std::cerr<<"Usage: el05_all_sine --confirm-hardware [--confirm-large-motion] [--hold-enabled] [--skip-id N] [--duration s] [--amplitude rad] [--frequency Hz] [--speed rad/s]\n"; return 2;}}
  ...
  try { for(int i=0;i<5;i++){if(!active[i]) continue; m[i]=std::make_unique<RobStrideMotor>("can0",0xFD,(uint8_t)(i+1),0); m[i]->Get_RobStrite_Motor_parameter(0x7019); c[i]=m[i]->drw.mechPos.data; if(!std::isfinite(c[i])) throw std::runtime_error("invalid position feedback");}
    ...
    while(!stop){double t=std::chrono::duration<double>(std::chrono::steady_clock::now()-start).count(); if(t>=duration)break; double q=amp*std::sin(2*M_PI*freq*t); for(int i=0;i<5;i++) if(active[i]) m[i]->RobStrite_Motor_PosCSP_control((float)speed,(float)(c[i]+q)); std::this_thread::sleep_for(std::chrono::milliseconds(20));}
