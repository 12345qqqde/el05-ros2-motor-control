...
void RobStrideMotor::receive_status_frame() {
  ...
  if (communication_type == Communication_Type_MotorRequest) {
    ...
    fault_value_ = (static_cast<uint32_t>(error_code) << 16) | (extra_data & 0xFFFF);
  ...
  } else if (communication_type == Communication_Type_ErrorFeedback) {
    if (data.size() >= 4)
      fault_value_ = static_cast<uint32_t>(data[0]) |
                     (static_cast<uint32_t>(data[1]) << 8) |
                     (static_cast<uint32_t>(data[2]) << 16) |
                     (static_cast<uint32_t>(data[3]) << 24);
    std::cerr << "Motor error feedback on CAN ID 0x" << std::hex
              << static_cast<unsigned int>(extra_data) << std::dec << ":";
    for (auto byte : data)
      std::cerr << " " << std::hex << static_cast<unsigned int>(byte);
    std::cerr << std::dec << std::endl;
  } else {
    std::cerr << "Ignoring unsupported communication type 0x" << std::hex
              << static_cast<unsigned int>(communication_type) << std::dec
              << std::endl;
  }
}
